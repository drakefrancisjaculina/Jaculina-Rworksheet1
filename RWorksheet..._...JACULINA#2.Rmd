...
#2 Create a vector using seq() function
    ...
    y = seq(1, 3, by=0.2)
    print(y)
    #It generated a 0.2 sequence from 1 to 3. It increases by 0.2 till it reaches 3.
```

```{r , include=TRUE}
#3 A factory has a census of its workers. There are 50 workers in total. 
#The following list shows their ages:
    #34, 28, 22, 36, 27,  18,  52,  39,  42,  29,  35,  31,  27, 22,  37,  34,
...
#4 Create a vector x <- c("first"=3, "second"=0, "third"=9). Then named the vector, names(x).
    ...
    x[c("first","third")]
    #After printing, a table with the first, second, and third values
    #redistributed to each character is provided. The same table is next, 
    #except for the second one is no longer exists.
```
...
#5 Create a sequence x from -3:2.
    ...
    x[2] <- 0
    print(x)
    #The sequence was 1 to 6 after changing x [2] into 0, whereas it is now 
    #1 0 3 4 5 6 after changing x [2] into 0. Now, 0 is in place of 2.
```
